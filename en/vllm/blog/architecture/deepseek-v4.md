...
2026-04-24. **vLLM Team**. Hugging Face: [`deepseek-ai/DeepSeek-V4-Pro`](https://huggingface.co/deepseek-ai/DeepSeek-V4-Pro) (1.6T) and [`deepseek-ai/DeepSeek-V4-Flash`](https://huggingface.co/deepseek-ai/DeepSeek-V4-Flash) (285B). Both claim up to **one million** tokens of context. Image in the recipes: `vllm/vllm-openai:deepseekv4-cu130`. Study note of the first support drop; further opts were still in flight on the page.

Earlier sparse-attention stack: [deepseek-v32.md](deepseek-v32.md). FP8 KV / attention path: [fp8-kvcache.md](../performance/fp8-kvcache.md). Wide-EP serving corridor: [large-scale.md](../serving/large-scale.md). GB200 scoreboard: [gb200-wideep.md](../serving/gb200-wideep.md).

The post’s three blocks: a quickstart; a first-principles walk through the new attention; then the vLLM systems work — hybrid KV cache, kernel fusion, disaggregated serving.
...
- **Attention compute.** Even with [DeepSeek Sparse Attention (DSA)](http://arxiv.org/abs/2512.02556), the matmul remains a bottleneck.

The V4 design compresses the cache **and** the compute:

1. **Share key and value vectors** (~**2×** memory savings). Correctness then needs an **inverse RoPE** on the attention output — full algebra in the appendix.
2. **Compress KV across tokens** (~**4× to 128×**). Two recipes:
...
- Same split as V3.2: attention kernel uses **bfloat16** KV for Prefill and **partially token-wise fp8** for Decode.
...
Each compressor layer keeps a small rolling residual per request: an **8-token** (overlapped) partial for C4, a **128-token** partial for C128. A per-request side buffer works in isolation and then fights the rest of the stack: prefix cache would have to snapshot the residual at every cacheable boundary; disaggregated Prefill would need a second transfer path for residuals.
...
- **Disaggregated Prefill.** Compressor state ships like SWA state: only blocks inside the window, no residual-specific path.
- **CUDA graphs** and **MTP** follow the same SWA integration pattern; metadata stays compressor-specific.

#### (3) Unifying page sizes

A C4 indexer block, a `c128a` KV block, and a `c4a` compressor-state block still have different **page sizes** (bytes per block). Separate pools would fragment each other.
...
Observed: **5–6%** end-to-end latency reduction at **low batch sizes**. CUDA graphs still cut launch overhead on Decode, as for every other model.
...
The drop targets NVIDIA **Hopper** and **Blackwell**; recipes on the [recipe site](https://recipes.vllm.ai/deepseek-ai/DeepSeek-V4-Pro). Hardware vendors can add support through the plugin door ([plugin-system.md](plugin-system.md) / [hardware-plugin.md](hardware-plugin.md)). Named independently: [vllm-ascend](https://github.com/vllm-project/vllm-ascend), [vllm-mlu](https://github.com/Cambricon/vllm-mlu).
...
Query at position $i$ after [RoPE](http://arxiv.org/abs/2104.09864): $<q_i, i> = R(i)q_i$, with $R(i)$ the rotation matrix at position $i$. Basic properties:

- $R(i)R(j) = R(i+j)$
- $R(i)^{-1} = R(i)^T = R(-i)$
- $R(i)$ is orthogonal: $R(i)R(i)^T = I$

Keys at $j_1, j_2, j_p, \ldots, j_n$: $<k_{j_p}, j_p> = R(j_p)k_{j_p}$. Values usually do **not** get RoPE: $<v_{j_p}, j_p> = v_{j_p}$.
...
Translation invariance: $R(j_p - i)$ depends only on relative position. Shift query and key by the same amount, output unchanged.
...
- **`c4a`:** the $j$-th compressed token is a weighted sum over $[4j - 4, 4j + 3]$ ($j$ from 0; negative indices treated as value 0). RoPE position: $4j$.
- **`c128a`:** weighted sum over $[128j, 128j + 127]$. RoPE position: $128j$.
