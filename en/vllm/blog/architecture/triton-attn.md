...
2026-03-04. Adapted from a Red Hat [vLLM Office Hours](https://www.youtube.com/watch?v=8QiM-i9ifFo&list=PLbMP1JcGBmSHxp4-lubU5WYmJ9YgAQcf3&index=1) session with **Burkhard Ringlein** (IBM Research). Playlist / future hours: [past topics](https://www.youtube.com/playlist?list=PLbMP1JcGBmSHxp4-lubU5WYmJ9YgAQcf3), [red.ht/office-hours](https://red.ht/office-hours). Authors on the effort: IBM Research, Red Hat, AMD. Study note. Attention-backend selection in the docs: [optimization.md](../../optimization/optimization.md).

Kernel: [`vllm/v1/attention/ops/triton_unified_attention.py`](https://github.com/vllm-project/vllm/blob/main/vllm/v1/attention/ops/triton_unified_attention.py) (~**800** LOC). FlashAttention 3 is ~**70,000** LOC. Backend wrapper: [`triton_attn.py`](https://github.com/vllm-project/vllm/blob/main/vllm/v1/attention/backends/triton_attn.py). Paper for the kernel: [*The Anatomy of a Triton Attention Kernel*](https://arxiv.org/abs/2511.11581). Autotuning paper named in Figure 1’s discussion: [*GPU Performance Portability needs Autotuning*](https://arxiv.org/abs/2505.03780).
...
The Triton backend bets on **performance-portable** kernels that adapt to the hardware they run on. [Triton](https://github.com/triton-lang/triton) is a DSL for writing GPU kernels (matmul, attention, …) in Python; they compile to efficient code on multiple platforms. Tiled programming: low-level enough for hardware-relevant optimizations, high-level enough to stay largely hardware-agnostic.
...
Inside that layer: FlashAttention and FlashInfer on CUDA, ROCm backends, specialized MLA backends (full list: [`vllm/v1/attention/backends`](https://github.com/vllm-project/vllm/tree/main/vllm/v1/attention/backends)). The Triton backend is **entirely Triton**, **native to vLLM**. Same source on NVIDIA, AMD, and Intel. Depends only on **PyTorch + Triton**. Always shipped with vLLM, so it is an **always-on fallback**. Started at IBM Research and Red Hat AI; now maintained more broadly.
...
- Models that need features such as **ALiBi sqrt** (StepFun audio models), **sink tokens** and **GPT-OSS** behavior — especially on **pre-Hopper NVIDIA** (A100s).
...
## Writing a high-performance portable paged-attention kernel
...
[Microbenchmarks](https://github.com/foundation-model-stack/vllm-triton-backend) were essential across Prefill-heavy, Decode-heavy, and mixed workloads, and across batch sizes and context lengths.

**Figure 2.** x-axis: total tokens; y-axis: latency. Separate subplots for prefill-only, mixed, and decode-only. Different kernel variants win in different regimes; **no single configuration dominates**. Microbenchmarks expose kernel-level behavior that end-to-end numbers hide.
...
GPUs run a fixed number of SMs. Launch more threads than SMs and execution proceeds in **waves**.

**Figure 5.** Example on the page: GPU has **8 SMs**, they want to execute **12 threads** — a second wave underutilizes.

Captured in a CUDA graph, that waste is **replayed** even if the effective workload shrinks.

**Figure 6.** Extra wasted work when replaying fixed launch grids via CUDA graphs → higher latency.
...
## Benchmarking results (late 2025)

**Figure 9** (two plots on the page: H100 and MI300; results normalized to the left-most baseline). End-to-end latency, **Llama 3.1 8B**, **batch size 1**, **input length 500** tokens. x-axis: output length. NVIDIA **H100** and AMD **MI300**.
