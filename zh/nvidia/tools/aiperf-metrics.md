...
Record 不知道整场；Derived 看不见单条 HTTP。所以「系统 TPS」和「这个请求觉得有多快」不是同一只秒表。
...
**ITL** — 稳态 token 间隔，**不含 TTFT**：

```
ITL = (request_latency − TTFT) / (output_sequence_length − 1)
```

至少两包非空，且 TTFT、request latency、OSL 都合法。这是 decode 阶段连续 token 之间的平均时间。

**ICL** — 相邻 **chunk** 到达间隔的整条分布，不是平均到 token。chunk 里可能挤了好几个 token。抖动、组 batch、网络抖动，看 ICL 比看一个平均 ITL 更诚实。

## 吞吐：系统 vs 单请求

**Output Token Throughput Per User** = `1 / ITL`（秒）。不含 TTFT。并发升高时，这个数字通常往下掉——整体更快，每一个请求更慢。
...
- **ISL**：prompt token
- **output_token_count**：给用户看的输出，不含独立 `reasoning_content` 字段；若思考写在普通 `content` 里（例如 think 标签），会算进去，除非另外滤
...
**Request throughput (RPS)** = `request_count / benchmark_duration`。请求有大有小，RPS 单独很少能说明问题。
...
吞吐告诉你每秒完成多少请求。Goodput 告诉你每秒有多少请求还满足 SLA。容量规划用后者，否则会按一个用户正在变慢的 TPS 去买卡。
...
HTTP 时序（`--show-trace-timing` / `--export-http-trace`）把一次请求拆成 sending / waiting（到第一字节 body）/ receiving。那是网络和握手，不是 TTFT 的同义词。教程：https://docs.nvidia.com/aiperf/tutorials/metrics-analysis/http-trace-metrics-guide
