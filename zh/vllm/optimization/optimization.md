...
官方建议的顺序是：先确认 **CPU 核够不够**（V1 是多进程，核不够时 GPU 会空等），再选优化等级 `-O0`～`-O3`（默认 `-O2`，cudagraph + fusion），再调 **`max_num_batched_tokens`**（小值更护 ITL，大值更护 TTFT；吞吐常要 >8192），然后才是 TP/PP/DP/EP 和 cache。显存不够时另见官方 memory conservation。旗标对应哪篇博客，见 [FLAG-MAP.md](../blog/FLAG-MAP.md)。

## 优化等级

四档，是启动时间和稳态速度之间的权衡：

- `-O0`：不优化。启动最快，跑得最慢。适合只想确认模型能加载。
- `-O1`：快优化。简单编译 + 快 fusion + PIECEWISE cudagraph。
- `-O2`：默认。更大编译范围、更多 fusion、FULL_AND_PIECEWISE cudagraph。
- `-O3`：名义上更激进。目前等于 `-O2`，以后可能加入更耗时或实验性的优化。

## 加快重复启动

同一套模型、配置、硬件反复启动时，不必每次都从头编译：

- **复用 compile cache。** `torch.compile` 产物在 `VLLM_CACHE_ROOT`（默认 `~/.cache/vllm`），可在机器间拷贝或打进镜像。`VLLM_FORCE_AOT_LOAD=1` 在 cache miss 时直接失败，而不是默默重编译。模型、配置、相关 `VLLM_*`、torch 版本、GPU 型号变了，cache 都会失效。
...
抢占能保证不崩，但伤害端到端延迟——被抢占的请求要把过去的计算重做一遍。频繁抢占时：
...
把大 prefill 切成小块，和 decode 请求组在同一个 batch 里：compute-bound 的 prefill 和 memory-bound 的 decode 可以一起跑。

V1 能开就默认开。调度：**先排所有 pending decode，再用 `max_num_batched_tokens` 预算去排 prefill**；单条 prefill 塞不下就自动切块。已经开始 decode 的请求优先继续。

好处：decode 优先，ITL 更好；同一 batch 里既有 prefill 又有 decode，GPU 更不容易空闲。
...
V1 是多进程架构。CPU 核不够是最常见、也最容易被误判成 GPU 问题的掉速原因，虚拟机里更明显。
...
CPU 不够时优先伤：输入处理吞吐、调度延迟、流式 detokenize/网络。GPU 利用率低于预期时，先查是不是 CPU 在饿着。
