...
生产 LLM serving 里，KV cache 是最贵的运行时资产之一。每机可以占掉 **几百 GiB**，分配和预热很慢，却常常比当时那波请求 **活得更久**。
...
内嵌 / 进程内：这 500 GiB 归 vLLM worker，vLLM **71.4 s** 才 ready。

池子预先进独立 PegaFlow server：server ready 之后 vLLM **33.2 s** → vLLM 启动快 **2.15×**，因为长命的主机分配不再跟推理进程绑死。

**Figure 2。** 500 GiB 主机 KV 池的 vLLM 启动时间。池子放在外部 PegaFlow 里，这条路径从 71.4 s 收到 33.2 s。

## Rust data path and tail-latency stability

把进程拆出去，首先是为了生命周期、共享、CPU 隔离。用 **Rust** 实现，顺手买到延迟稳定性。

数据面躲开 Python 解释器、**GIL** 争用、停顿式 GC。生产 cache 不只在关键路径搬字节：统计、索引上传、预取、健康检查、指标、驱逐、SSD 管理。这些任务住在同一份独立的 Rust 服务里，**不和 vLLM 共用一个解释器**。控制面和维护活可以跑，数据面少被打扰。
...
生产里，同一份逻辑 KV 常常被存很多遍，因为进程、模型、节点把 cache 彼此挡住。原文点名的几种：
...
单机上，本地实例都连同一台 PegaFlow，共享一份 CPU KV 池。小模型多实例、WideEP 的 DP replica、TP worker：相同 block 可以 **物理上只存一次**，给多个引擎复用。
...
PegaFlow 用三层：热的本地 block 留在 pinned DRAM；远端命中走 RDMA；更冷但仍可复用的溢到本地 SSD。
...
只看在线命中率会骗人。负载几乎没有复用时，**3%** 可能已经很好；理论上限高很多时，**90%** 仍可能有空档。

运维该问的不是「命中率是多少」，而是：**离这份负载合理能达到的最好命中，我们还有多远？**
...
滚动 HLL 窗口，默认：**15 分钟**、**1 小时**、**24 小时**。把实测命中和 `r*` 放在同一块仪表上，分三种情形：
...
接 vLLM（文中例子 `vllm>=0.20.0`），不改 vLLM 源码：
...
GitHub：[novitalabs/pegaflow](https://github.com/novitalabs/pegaflow)。仓库里还有安装、server 配置、P2P RDMA、指标、vLLM connector 例子。
