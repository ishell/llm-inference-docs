...
普通负载均衡把 LLM 当无状态 HTTP。KV 是有状态的——下一句还想落到仍握着这段 cache 的 worker。Prefill/Decode 分离更不是「所有 pod 长得一样」：一边是 compute-bound 的 Prefill，一边是 memory-bound 的 Decode。Router 要办的就是这两件事。production-stack 里的 prefix-aware routing 是同一直觉的 Helm 版；这一篇把它收成一只独立网关。
...
- Round robin / random：无状态兜底——benchmark 里的对照，也是「完全没有共享前缀」时的老实选择。
...
数字是这一天、这一套拓扑上的。K8s 原生 RR 不认 P/D，当对照很合适，拿去贬低 2026 年的 llm-d 不合适。
...
读完 production-stack / AIBrix 再读这一篇：集群里真正决定「下一句话去哪张卡」的，是这只认得 KV 的路由器。P/D 的编排从这里开始；下一篇 EPD 把**视觉编码器**也拆出去。
