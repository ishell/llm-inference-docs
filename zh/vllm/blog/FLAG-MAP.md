...
| `-O0`…`-O3`、CUDA graph、`torch.compile` | 优化等级；`--enforce-eager` | [v0.6](performance/v0.6-throughput.md)、[V1](architecture/v1-alpha.md)、[MRV2](architecture/mrv2.md) | 启动时间和稳态 decode 的交易。 |
...
| prefix cache | features 页 | Anatomy、[production-stack](serving/production-stack.md)、[Router](serving/router.md)、[Mooncake](serving/mooncake.md) | 本机命中 → 粘会话；粘不住 → 分布式池。 |
| speculative decoding | features 页 | [spec-decode](performance/spec-decode.md) | draft/verify；文中「还不支持」是历史。 |
| FP8 KV / attention 量化 | 显存与精度 | [FP8 KV](performance/fp8-kvcache.md) | Hopper/Blackwell 上的验证，不是「开了就快」。 |
...
| KVConnector / 外置 KV | （博客） | Mooncake、production-stack | 同一扇插件门：LMCache、Mooncake Store、NIXL。 |
