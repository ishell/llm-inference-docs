...
LLM 推理要 CPU 和 GPU 一起转。大矩阵在 GPU 上；CPU 负责收请求、调度、准备输入。CPU 跟不上，GPU 就闲着等——利用率掉下去，推理也就慢了。
...
1. **CPU 开销高。** 为了好懂、好贡献，大部分逻辑留在 Python，调度和准备输入用 Python 原生的 List / Dict。那笔开销会显形：调度慢，数据准备也慢。
2. **组件之间缺少异步。** scheduler 和 output processor 会同步地堵住 GPU。他们点名两个原因：当年假设「模型执行一定比 CPU 慢」；beam search 这类复杂调度更好写。GPU 于是经常空等。

一句话：挡住 GPU 的，是 **CPU 开销**。v0.6.0 用一个月来砍这笔开销。
...
调度器和输入准备太慢，GPU 步与步之间会空转，吞吐就次。**multi-step scheduling**：调度和准备只做一次，模型连跑 `n` 步 GPU。这 `n` 步中间 GPU 不必等 CPU。CPU 的开销摊到这 n 步上，空转少了，整体就快了。
...
新环：**asynchronous output processing**——把第 `n` 步的输出处理叠到第 `n+1` 步的 GPU 上。不立刻处理，先假定第 n 步没人结束。代价是每个请求**多跑一步**。GPU 利用率补得回来。
...
- **简单采样快路径**（[#7117](https://github.com/vllm-project/vllm/pull/7117)）：vLLM 支持多种 attention backend 和采样算法。常见负载、简单采样，跳过完整路径。
...
ShareGPT 和 Decode-heavy 上，运行 Llama-3 时，他们自称 **H100 上吞吐最高**。
...
读这一篇，是为了记住一句不好听的话：**卡很快以后，慢的是 Python。** V1 把这句话写成了架构。
