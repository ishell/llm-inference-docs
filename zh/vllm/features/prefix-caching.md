...
打开：`enable_prefix_caching=True`，或 `vllm serve` 的 `--enable-prefix-caching`。哈希默认 `sha256`（`--prefix-caching-hash-algo`）。跨环境要可复现用 `sha256_cbor`；`xxhash` / `xxhash_cbor` 更快，碰撞隔离更弱——多租户先读 [设计页](prefix-caching-design.md) 的安全注。离线示例在官方树 `examples/features/automatic_prefix_caching/automatic_prefix_caching_offline.py`。
