...
目录里的官方 URL **几乎都有本地笔记**，但不是全文下载、也不是官方译本。vLLM 博客全表在 [en/vllm/blog/CATALOG.md](en/vllm/blog/CATALOG.md)；带日期的 **134 篇都有中英学习译文**（按原文分节写完，不是压缩 stub）。[必读列表](zh/vllm/blog/MUST-READ.md) 只排主线 + 机制波次，不必按 CATALOG 逐篇读。`vllm serve` 只摘了性能相关旗标。NVIDIA 一侧仍是压缩对照，篇幅往往短于英文网页。版权也不允许把官方页面原样搬进来。
...
| blog | [其余 134 篇](zh/vllm/blog/README.md) | [en/vllm/blog/CATALOG.md](en/vllm/blog/CATALOG.md) | 学习译文（按原文分节）；全表见博客 README |
